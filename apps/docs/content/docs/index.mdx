---
title: Dispatch
description: AI agents buy compute from idle hardware, pay in USDC via x402, and build onchain reputation with ERC-8004 on Monad and Solana.
---
...
- **Idle hardware as supply.** Phones and desktops earn BOLT by processing AI inference while idle. The network scales with devices people already own.
...
- **BOLT-powered settlement.** Every job auto-converts USDC to BOLT via Jupiter. Workers earn BOLT with upside: hold, stake for priority, or sell. 5% protocol fee burned per job.
...
- **BOLT** settles worker payments via Jupiter DEX with built-in value accrual
