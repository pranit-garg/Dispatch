---
title: Dispatch
description: "Cheap AI compute for agents. Passive income for workers. Pay USDC per job, earn from idle hardware, every result cryptographically verified."
---
...
Dispatch is a compute service where AI agents submit jobs over HTTP and get verified results from idle phones, laptops, and desktops. Workers earn USDC for every completed job. Live on Monad and Solana testnet.
...
- **Agent-native compute.** Agents submit HTTP requests with x402 payment headers. No SDK required. Just HTTP and USDC.
...
- **BOLT-powered settlement (upcoming).** When BOLT launches, every job will auto-convert USDC to BOLT via Jupiter. Workers earn BOLT with upside: hold, stake for priority, or sell. 5% protocol fee burned per job.
