---
title: Run a Worker
description: Set up and run a desktop worker node on the Dispatch network, including keypair generation, registration, and trust pairing.
---
...
- **Node.js** 20+
- **pnpm** 9+
- **Ollama** installed and running (for LLM inference jobs). Install from [ollama.com](https://ollama.com).
...
The Dispatch CLI provides a unified interface for worker operations:

### Start a worker

```bash
dispatch worker start --coordinator http://localhost:4010 --capabilities llm,task
```
...
```bash
dispatch worker register --name "My Worker" --private-key $WORKER_PRIVATE_KEY
```

### With trust pairing

```bash
dispatch worker start --trust-code A3F2B1
```

> The CLI is under active development. Use the pnpm commands above for current setup.
