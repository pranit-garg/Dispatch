---
title: x402 Payments
description: HTTP-native USDC micropayments on every compute job via the x402 open protocol. No custom token, no staking.
---

# x402 Payments

Dispatch uses [x402](https://www.x402.org/), an open HTTP-native payment protocol, to settle USDC micropayments on every compute job. No custom token, no staking, no bridging. Workers get paid the moment a job completes.
