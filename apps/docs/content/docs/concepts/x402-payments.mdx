---
title: x402 Payments
description: HTTP-native USDC micropayments on every compute job via Coinbase's x402 protocol. No custom token, no staking.
---

# x402 Payments

Dispatch uses [x402](https://www.x402.org/), Coinbase's open HTTP-native payment protocol, to settle USDC micropayments on every compute job. No custom token, no staking, no bridging. Workers get paid the moment a job completes.
...
## Protocol Fee + BOLT Settlement

After x402 verifies the USDC payment, the coordinator triggers BOLT settlement:

1. **USDC received.** x402 facilitator confirms payment.
2. **Jupiter swap.** USDC auto-converts to BOLT via Jupiter DEX.
3. **Protocol fee.** 5% of BOLT burned (permanently removed from supply).
4. **Worker payment.** Remaining 95% BOLT transferred to worker's wallet.

This means 100% of economic activity flows through BOLT. Every job creates buy pressure on the token, not just a fee skim.

Workers choose what to do with earned BOLT: hold for upside, stake for priority matching, or sell back to USDC on Jupiter.
